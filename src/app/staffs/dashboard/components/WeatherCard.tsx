...
import { getFarmProfile } from "@/lib/services/farm";
...
export default function WeatherCard() {
  ...
  useEffect(() => {
    ...
    const getAsyncWeather = async () => {
      setIsLoading(true);
      try {
        const profileData = await getFarmProfile(user?.farmId || "");
        const formattedState =
          profileData?.location?.state.charAt(0).toUpperCase() +
          profileData?.location?.state.slice(1);
        ...
        if (
          ...
          res.data.name.toLowerCase() !==
            profileData?.location?.state.toLowerCase()
        ) {
    ...
    if (!weatherInfo) {
      return (
        <div className="flex flex-col items-center justify-center h-full min-h-[250px] text-center">
          ...
          <p className="text-gray-500 text-sm">
            Could not fetch weather for {displayLocation}.
          </p>
        </div>
      );
    }
