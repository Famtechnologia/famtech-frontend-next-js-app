...
export const AddLivestockForm: React.FC<AddLivestockFormProps> = ({
  onClose,
  onRecordAdded,
}) => {
  ...
  const handleChange = (
    ...
    setFormData((prev) => {
      ...
      if (id === "healthStatus") {
        const healthValue =
          value.toLowerCase() as LivestockFormData["healthStatus"];
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    data.append("userId", user?._id || profile?.id || "");
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div className="sm:col-span-2">
          ...
          <input
            type="text"
            id="specie"
            placeholder="E.g., Cattle, Chicken, Goat"
        ...
        <div>
          ...
          <input
            type="text"
            id="feedSchedule"
            placeholder="E.g twice a day, grass"
...
// 🚀 Add Auth Context for user ID

// --- Component ---
export const UpdateLivestockForm: React.FC<UpdateLivestockFormProps> = ({
  record,
  onClose,
  onRecordUpdated,
}) => {
  ...
  const [formData, setFormData] = useState<UpdateLivestockPayload>({
    ...
    healthStatus: (record.healthStatus?.toLowerCase() ||
      "good") as UpdateLivestockPayload["healthStatus"],
    note: record.note || "",
  });
  ...
  const handleChange = (
    ...
    setFormData((prev) => {
      ...
      if (id === "healthStatus") {
        const healthValue =
          value.toLowerCase() as UpdateLivestockPayload["healthStatus"];
        return { ...prev, [id]: healthValue };
      }
