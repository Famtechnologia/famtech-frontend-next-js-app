        ...
        <div className="space-y-6">
                    ...
                    images.map((img, idx) => {
                        ...
                        if (!url) {
                            return (
                                <div
                                    key={idx}
                                    className="w-32 h-32 flex items-center justify-center bg-gray-100 text-gray-400 rounded-md text-xs"
                                >
                                    No image
                                </div>
                            );
                        ...
                        return (
                            <div key={idx} className="w-32 h-32 relative flex-shrink-0">
                                <Image
                                    ...
                                    alt={
                                        isCrop
                                            ? `${crop.cropName} image ${idx + 1}`
                                            : `${livestock.specie} image`
                                    }
                                    fill
                                    sizes="128px"
            ...
            {isCrop ? (
                <div className="text-sm space-y-2 p-1">
                    ...
                    <p>
                        <b>Expected Harvest:</b>{" "}
                        {new Date(crop.expectedHarvestDate).toLocaleDateString()}
                    </p>
                    <p>
                        <b>Growth Stage:</b> {crop.currentGrowthStage}
                    </p>
                    <p>
                        <b>Health Status:</b>{" "}
                        <span
                            className={`capitalize font-medium ${crop.healthStatus?.toLowerCase() === "poor" ? "text-red-600" : "text-green-600"}`}
                        >
                            {crop.healthStatus}
                        </span>
                    </p>
                    <p>
                        <b>Area:</b> {crop.area?.value} {crop.area?.unit}
                    </p>
                    <p>
                        <b>Seed Quantity:</b> {crop.seedQuantity?.value}{" "}
                        {crop.seedQuantity?.unit}
                    </p>
                    <p>
                        <b>Note:</b> {crop.note || "N/A"}
                    </p>
                </div>
            ) : (
                <div className="text-sm space-y-2 p-1">
                    <p>
                        <b>Breed:</b> {livestock.breed || "N/A"}
                    </p>
                    <p>
                        <b>Number of Animals:</b> {livestock.numberOfAnimal}
                    </p>
                    <p>
                        <b>Acquisition Date:</b>{" "}
                        {new Date(livestock.acquisitionDate).toLocaleDateString()}
                    </p>
                    <p>
                        <b>Health Status:</b>{" "}
                        <span
                            className={`capitalize font-medium ${livestock.healthStatus?.toLowerCase() === "poor" ? "text-red-600" : "text-green-600"}`}
                        >
                            {livestock.healthStatus ?? "N/A"}
                        </span>
                    </p>
                    <p>
                        <b>Feed Schedule:</b> {livestock.feedSchedule ?? "N/A"}
                    </p>{" "}
                    <p>
                        <b>Note:</b> {livestock.note || "N/A"}
                    </p>
                </div>
            )}
...
const CropLivestockRecords: React.FC = () => {
    ...
    const fetchCropData = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const data: BaseCropRecord[] = await getCropRecords(userId);
    ...
    const fetchLivestockData = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const data: BaseLivestockRecord[] = await getLivestockRecords(userId);
        ...
        <div className="p-2 lg:p-6">
            ...
            {error && (
                <div className="text-center py-8 text-red-500">
                    Error: Could not fetch records. Please check your network and try
                    again.
                </div>
            )}
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="relative p-4">
                                    {/* Health Status Badge */}
                                    <div
                                        className={`absolute top-0 right-4 -translate-y-1/2 px-3 py-1 rounded-full text-xs font-semibold text-white capitalize ${getHealthStatusColor(recordStatus)}`}
                                    >
                                        {recordStatus || "N/A"}
                                    </div>
                                    ...
                                    {isCurrentCrop ? (
                                        ...
                                        <div className="mt-4 text-sm text-gray-600 space-y-2">
                                            <p>
                                                <b>Age:</b> {livestockRecord.ageGroup}
                                            </p>
                                            {/* ADDED Feed Schedule to Card View */}
                                            <p>
                                                <b>Feed Schedule:</b>{" "}
                                                <span className="text-gray-800 font-semibold">
                                                    {livestockRecord.feedSchedule || "N/A"}
                                                </span>
                                            </p>
                                            <p>
                                                <b>Last Checkup:</b>{" "}
                                                {new Date(
                                                    livestockRecord.acquisitionDate
                                                ).toLocaleDateString()}
                                            </p>
                                        </div>
                                    )}
