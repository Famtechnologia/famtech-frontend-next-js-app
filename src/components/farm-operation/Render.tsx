...
type InventoryFormData =
  | Omit<UnifiedInventoryItem, "id" | "timestamp" | "userId">
  | UnifiedInventoryItem;
  ...
  return (
    <div className="space-y-1.5">
      ...
      <input
        ...
        value={displayValue}
        onChange={onChange}
        className="block w-full rounded-xl border border-gray-200 shadow-sm px-4 py-2.5 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-4 focus:ring-green-100/50 focus:border-green-600 transition-all"
        required={required}
...
const ToolSpecificFields = ({
  ...
  return (
    <div className="border-t pt-4 mt-4 border-gray-200 space-y-3">
      ...
      <InputField
        label="Tool Type"
        name="toolData.toolType"
        value={toolData.toolType}
        onChange={handleChange}
        placeholder="e.g., Handheld, Motorized"
      />
      <InputField
        label="Brand"
        name="toolData.brand"
      ...
      <InputField
        ...
        name="toolData.lastServiced"
        type="date"
        value={
          toolData.lastServiced
            ? new Date(toolData.lastServiced).toISOString().split("T")[0]
            : ""
        }
        onChange={handleChange}
      />
      <InputField
        ...
        name="toolData.warrantyExpiry"
        type="date"
        value={
          toolData.warrantyExpiry
            ? new Date(toolData.warrantyExpiry).toISOString().split("T")[0]
            : ""
        }
        onChange={handleChange}
      />
...
const EquipmentPartSpecificFields = ({
  equipmentPartData,
  handleChange,
}: {
  ...
  return (
    <div className="border-t pt-4 mt-4 border-gray-200 space-y-3">
      ...
      <InputField
        label="Part Number"
        name="equipmentPartData.partNumber"
        value={equipmentPartData.partNumber}
        onChange={handleChange}
        placeholder="e.g., 7789-A"
      />
      <InputField
        label="Manufacturer"
        name="equipmentPartData.manufacturer"
        value={equipmentPartData.manufacturer}
        onChange={handleChange}
      />
      <InputField
        label="Condition"
        name="equipmentPartData.condition"
        value={equipmentPartData.condition}
        onChange={handleChange}
      ...
      <InputField
        label="Warranty Expiry Date"
        name="equipmentPartData.warrantyExpiry"
        type="date"
        value={equipmentPartData.warrantyExpiry}
        onChange={handleChange}
      />
    </div>
  );
...
export const renderFormFields = (
  ...
  return (
    <div className="space-y-4">
      ...
      {isConsumable && (
        <ConsumableFields data={data} handleChange={handleChange} />
      )}
      ...
      {data.category === "feed" && (
        <InputField
          ...
          value={data.type}
          onChange={handleChange}
          placeholder="e.g., Concentrate, Hay"
        />
      )}
